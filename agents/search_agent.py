...
def retrieve_papers(sub_queries: list[str]) -> list[Paper]:
    """
    Search OpenAlex for each sub-query and return deduplicated papers.

    Deduplication strategy:
        - Primary:   by DOI (if available)
        - Fallback:  by exact title match (for papers without DOI)
    ...
    for query in sub_queries:
        print(f"[SearchAgent] Searching OpenAlex for: '{query}'")
        ...
        try:
            ...
            print(f"[SearchAgent] ERROR fetching '{query}': {e}")
            continue
        ...
        print(f"[SearchAgent] Added {added} new papers (total so far: {len(all_papers)})")
